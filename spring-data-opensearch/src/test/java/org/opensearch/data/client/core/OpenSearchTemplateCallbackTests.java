...
abstract class OpenSearchTemplateCallbackTests {
    ...
    @Mock
    protected org.opensearch.search.SearchHit searchHit;

    private final IndexCoordinates index = IndexCoordinates.of("index");

    @Spy
    private ValueCapturingAfterSaveCallback afterSaveCallback = new ValueCapturingAfterSaveCallback();

    @Spy
    private ValueCapturingAfterConvertCallback afterConvertCallback = new ValueCapturingAfterConvertCallback();

    @Spy
    private ValueCapturingBeforeConvertCallback beforeConvertCallback = new ValueCapturingBeforeConvertCallback();

    protected final void initTemplate(AbstractElasticsearchTemplate template) {
        this.template = template;
    }

    protected final org.opensearch.search.SearchHits nSearchHits(int count) {
        org.opensearch.search.SearchHit[] hits = new org.opensearch.search.SearchHit[count];
        Arrays.fill(hits, searchHit);
        return new org.opensearch.search.SearchHits(hits, new TotalHits(count, TotalHits.Relation.EQUAL_TO), 1.0f);
    }
    ...
    void multiSearchWithMultipleEntityClassesShouldInvokeAfterConvertCallback() {
        ...
        List<SearchHits<?>> results =
                template.multiSearch(singletonList(queryForTwo()), singletonList(Person.class), index);
