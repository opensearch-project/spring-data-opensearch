...
public class RestClientsTest {
    ...
    void shouldConfigureClientAndSetAllRequiredHeaders(ClientUnderTestFactory clientUnderTestFactory) {
        ...
        wireMockServer(server -> {
            ...
            var configurationBuilder = ClientConfiguration
                    .builder()
            ...
            if (clientUnderTestFactory instanceof OSCUnderTestFactory) {
                configurationBuilder.withClientConfigurer(
                        OpenSearchClients.OpenSearchHttpClientConfigurationCallback.from(httpClientBuilder -> {
                            httpClientConfigurerCount.incrementAndGet();
                            return httpClientBuilder;
                        }));
                configurationBuilder.withClientConfigurer(
                        OpenSearchClients.OpenSearchRestClientConfigurationCallback.from(restClientBuilder -> {
                            restClientConfigurerCount.incrementAndGet();
                            return restClientBuilder;
                        }));
            } else if (clientUnderTestFactory instanceof ReactiveOSCUnderTestFactory) {
                configurationBuilder
                        .withClientConfigurer(OpenSearchClients.OpenSearchHttpClientConfigurationCallback.from(webClient -> {
                            httpClientConfigurerCount.incrementAndGet();
                            return webClient;
                        }));
                configurationBuilder.withClientConfigurer(
        ...
        wireMockServer(server -> {
            ...
            stubFor(put(urlMatching(urlPattern)) //
                    .willReturn(jsonResponse("""
                            ...
                            """ //
                            , 201) //
                            .withHeader("Content-Type", "application/vnd.opensearch+json;compatible-with=7")));

            ClientConfiguration clientConfiguration = ClientConfiguration.builder()
                    .connectedTo("localhost:" + server.port()) //
                    .withHeaders(() -> {
                        HttpHeaders defaultCompatibilityHeaders = new HttpHeaders();
                        defaultCompatibilityHeaders.add("Accept", "application/vnd.opensearch+json;compatible-with=7");
                        defaultCompatibilityHeaders.add("Content-Type", "application/vnd.opensearch+json;compatible-with=7");
            ...
            verify(putRequestedFor(urlMatching(urlPattern)) //
                    .withHeader("Accept", new EqualToPattern("application/vnd.opensearch+json;compatible-with=7")) //
                    .withHeader("Content-Type", new EqualToPattern("application/vnd.opensearch+json;compatible-with=7")) //
            );
        });
    }
    ...
    /**
     * {@link ClientUnderTestFactory} implementation for the {@link OpenSearchClient}.
     */
    static class OSCUnderTestFactory extends ClientUnderTestFactory {
        ...
        ClientUnderTest create(ClientConfiguration clientConfiguration) {
            ...
            OpenSearchClient client = OpenSearchClients.createImperative(clientConfiguration);
    ...
    /**
     * {@link ClientUnderTestFactory} implementation for the {@link ReactiveOpenSearchClient}.
     */
    static class ReactiveOSCUnderTestFactory extends ClientUnderTestFactory {

        @Override
        protected String getDisplayName() {
            return "ReactiveOpenSearchClient (OSC based)";
        }
        ...
        ClientUnderTest create(ClientConfiguration clientConfiguration) {
            ...
            ReactiveOpenSearchClient client = OpenSearchClients.createReactive(clientConfiguration);
            return new ClientUnderTest() {
                @Override
                public boolean ping() throws Exception {
    ...
    static Stream<ClientUnderTestFactory> clientUnderTestFactorySource() {
        return Stream.of( //
                new OSCUnderTestFactory(), //
                new ReactiveOSCUnderTestFactory());
    }
}
