...
class CriteriaFilterProcessor {
    ...
    QueryBuilder createFilter(Criteria criteria) {
        ...
        for (Criteria chainedCriteria : criteria.getCriteriaChain()) {
            ...
            if (chainedCriteria.isOr()) {
                ...
                List<QueryBuilder> negationFilters = buildNegationFilter(
                        criteria.getField().getName(),
    ...
    private QueryBuilder queryFor(OperationKey key, Object value, String fieldName) {
        ...
        switch (key) {
            case WITHIN:
                Assert.isTrue(
                        value instanceof Object[], "Value of a geo distance filter should be an array of two values.");
            ...
            case GEO_IS_DISJOINT:
                Assert.isTrue(
                        value instanceof GeoJson<?>, "value of a GEO_IS_DISJOINT filter must be a GeoJson object");
    ...
    private QueryBuilder withinQuery(String fieldName, Object[] valArray) {
        ...
        Assert.isTrue(
                valArray[1] instanceof String || valArray[1] instanceof Distance,
        ...
        if (valArray[0] instanceof GeoPoint) {
            ...
            if (loc.contains(",")) {
                String[] c = loc.split(",");
                filter.point(Double.parseDouble(c[0]), Double.parseDouble(c[1]))
    ...
    private void oneParameterBBox(GeoBoundingBoxQueryBuilder filter, Object value) {
        Assert.isTrue(
                value instanceof GeoBox || value instanceof Box,
        ...
        filter.setCorners(
                geoBBox.getTopLeft().getLat(),
                geoBBox.getTopLeft().getLon(),
                geoBBox.getBottomRight().getLat(),
    ...
    private void twoParameterBBox(GeoBoundingBoxQueryBuilder filter, Object[] values) {
        Assert.isTrue(
                isType(values, GeoPoint.class) || isType(values, String.class),
